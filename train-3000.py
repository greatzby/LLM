...
# ---- Plot only the data for the first 3000 iterations ----

# Filter training history data for iterations <= 3000
train_iter_history_short = []
train_loss_history_short = []
for iter_val, loss_val in zip(train_iter_history, train_loss_history):
    if iter_val <= 3000:
        train_iter_history_short.append(iter_val)
        train_loss_history_short.append(loss_val)

# Filter testing history and accuracy data for iterations <= 3000
test_iter_history_short = []
tf_accuracy_history_short = []
ar_accuracy_history_short = []
for iter_val, tf_acc, ar_acc in zip(test_iter_history, tf_accuracy_history, ar_accuracy_history):
    if iter_val <= 3000:
        test_iter_history_short.append(iter_val)
        tf_accuracy_history_short.append(tf_acc)
        ar_accuracy_history_short.append(ar_acc)

plt.figure(figsize=(12, 5))
plt.subplot(1, 2, 1)
plt.plot(train_iter_history_short, train_loss_history_short, marker='o')
plt.xlabel('Iteration')
plt.ylabel('Training Loss')
plt.title('Training Loss Curve (Iter <= 3000)')
plt.subplot(1, 2, 2)
plt.plot(test_iter_history_short, tf_accuracy_history_short, marker='o', label='Teacher Forcing')
plt.plot(test_iter_history_short, ar_accuracy_history_short, marker='o', label='Autoregressive (Lenient)')
plt.xlabel('Iteration')
plt.ylabel('Accuracy')
plt.title('Evaluation Accuracy (Iter <= 3000)')
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(out_dir, "training_curves_compare_Lenient_3000.png"))
plt.show()