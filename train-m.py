...
def evaluate_autoregressive_lenient():
    """
    使用自回归生成方式对测试集（test.txt）计算宽松规则下的准确率
    """
    ...
    num_correct = 0
    batch_size_eval = 100  # 分批评估
    for i in range(0, num_total, batch_size_eval):
        batch_lines = test_lines[i: i+batch_size_eval]
        encoded_texts = [encode(line) for line in batch_lines]
        if len(encoded_texts) == 0:
            continue
        max_len = max(len(seq) for seq in encoded_texts)
        padded = [seq + [0]*(max_len - len(seq)) for seq in encoded_texts]
        ...
        for j in range(test_batch.shape[0]):
            ...
            gen_str = decode(gen_tokens)
            symbol = check_path(G, gen_str)
            if symbol == "":
                num_correct += 1
...
while True:
    ...
    if iter_num % test_interval == 0 and master_process:
        ...
        ar_acc = evaluate_autoregressive_lenient()  # autoregressive 模式 (宽松规则)
